defmodule Wordle do
  ...
  Returning a list where exact matches are represented as ``[:green]``,
  occurrences are represented as ``[:yellow]`` (prioritizing first appearances)
  and no matches are represented as ``[:gray]``.

  Return a ``error`` if answer and gray are not binary with same length.
